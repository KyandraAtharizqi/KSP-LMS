...
<div class="container">
    <h4 class="mb-4">
        {{ isset($evaluasi) ? 'Edit' : 'Isi' }} Evaluasi Level 3 - Peserta
    </h4>
    ...
    <form 
        action="{{ isset($evaluasi)
            ? route('evaluasi-level-3.peserta.update', [$pelatihan->id, $evaluasi->id])
            : route('evaluasi-level-3.peserta.store', $pelatihan->id) }}"
        method="POST">
        @csrf
        @if(isset($evaluasi))
            @method('PUT')
        ...
        <table class="table table-bordered" id="action-plan-table">
            ...
            <tbody>
                @if(isset($evaluasi) && $evaluasi->actionPlans->count())
                    @foreach($evaluasi->actionPlans as $i => $plan)
                        <tr data-row-index="{{ $i }}">
                            <td>
                                <input type="text" name="action_plan[{{ $i }}]" class="form-control"
                                    value="{{ old('action_plan.'.$i, $plan->action_plan) }}" required>
                            </td>
...
document.addEventListener('DOMContentLoaded', function() {
    let actionPlanRowCounter = {{ isset($evaluasi) ? $evaluasi->actionPlans->count() : 1 }};
